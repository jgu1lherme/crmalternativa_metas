...
def processar_vendas(
    ...
    df_vendas = pd.read_excel(arquivo_vendas, dtype={"PED_EMISSAO": str})
    df_vendas["PED_EMISSAO"] = pd.to_datetime(df_vendas["PED_EMISSAO"], errors="coerce")

    if df_vendas["PED_EMISSAO"].isna().all():
    ...
    if data_inicial and data_final:
        df_vendas = df_vendas[
            (df_vendas["PED_EMISSAO"] >= pd.Timestamp(data_inicial))
            & (df_vendas["PED_EMISSAO"] <= pd.Timestamp(data_final))
        ]
    elif mes_referencia:
        df_vendas = df_vendas[df_vendas["PED_EMISSAO"].dt.month == mes_referencia]
...
uploaded_file = st.file_uploader("📂 Envie a planilha de vendas (1316)", type=["xlsx"])
