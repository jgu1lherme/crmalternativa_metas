...
def gerar_tabela_diaria_empresa(df_vendas_filtrado):
    """Gera uma tabela com o total de vendas da empresa por dia."""
    if df_vendas_filtrado is None or df_vendas_filtrado.empty:
    ...
    # Pivotar para ter colunas OPD e Distribuição por dia
    tabela = pd.pivot_table(
        df_validos,
        values='PED_TOTAL',
        index=df_validos['DAT_CAD'].dt.date, # Agrupa pela data
        columns='Tipo Venda',
        aggfunc=np.sum,
        fill_value=0
    )
    ...
    # Calcular total diário e ordenar
    tabela['Total Dia'] = tabela['OPD'] + tabela['Distribuição']
    tabela = tabela.sort_index(ascending=True) # Ordena pela data (índice)

    # Adicionar linha de Total Geral ANTES de formatar o índice
    total_geral = tabela.sum().to_frame().T
    total_geral.index = ['**TOTAL GERAL**'] # Usamos uma string como índice
    tabela = pd.concat([tabela, total_geral])

    # Formatar o índice (Data) APÓS adicionar o total
    tabela.index = [idx.strftime('%d/%m/%Y') if isinstance(idx, datetime.date) else idx for idx in tabela.index]

    # Resetar o índice para torná-lo uma coluna
...
def gerar_tabela_vendedor(df_vendas_filtrado):
    """Gera uma tabela detalhada das vendas de um vendedor E RETORNA OS TOTAIS.""" # Modificado
    if df_vendas_filtrado is None or df_vendas_filtrado.empty:
        st.info("Nenhuma venda encontrada para este vendedor no período.")
        return pd.DataFrame(), {} # Retorna DF vazio e dict vazio
    ...
    # *** CÁLCULO DOS TOTAIS ANTES DE FORMATAR ***
    ...
    tabela = df_validos[[ # Usar df_validos aqui
        'DAT_CAD',
        'VEN_NOME',
        'CLI_RAZ',
        'PED_TOTAL',
    ...
    return tabela_final, totais # Retorna a tabela E os totais
...
# ==================================================================
    ...
    # ==================================================================
        ...
        if df_filtrado is None or df_filtrado.empty:
            st.warning("Nenhuma dado para exibir nos Relatórios com os filtros atuais.")
        else:
            # Se "Todos" estiver selecionado, oferece duas visões
            if vendedor_selecionado == "Todos":
            ...
            # Se um vendedor específico estiver selecionado, mostra a tabela detalhada E OS TOTAIS
            else:
                ...
                if not tabela_detalhada.empty:
                    ...
                    col2.metric("Total Distribuição", f"R$ {totais_vendedor.get('Distribuição', 0):,.2f}")
                    col3.metric("💰 Total Geral Vendedor", f"R$ {totais_vendedor.get('Total', 0):,.2f}")
                    # ------------------------------
